...
## only the ones that fluctuate smaller than 5%
```{r data_processing}
...
for (i in ticker_processed) {
    ...
    if (count(l)<dim(trades$df.tickers[which(trades$df.tickers$ticker == "QQQ"),])[1]) next
      ...
      if ((max(tail(l_boll$BBands,15))< (1+price_fluctuation)*l_boll$BBands[length(l_boll$BBands)-15]) & (min(tail(l_boll$BBands,15)) > (1-price_fluctuation)*l_boll$BBands[length(l_boll$BBands)-15])) {
...
trades_picked <- mutate(trades_processed_boll, 
                        BBands = ticker_boll_picked$BBands, 
                        Bdn = ticker_boll_picked$Bdn, 
                        Bup = ticker_boll_picked$Bup,
                        pctB = ticker_boll_picked$pctB,
                        Width = ticker_boll_picked$Width)

```

## Get symbols that have large 40% price drops within 2 months
```{r filter_price_drop}
...
write.xlsx(ticker_picked2,"./Stock_picked2.xlsx")
```
