...
ggplot(ABS_plot,aes(year,freq,group = word,color = word)) + geom_line()+geom_point(aes(shape = word,size = 2))+theme(axis.text.x = element_text(size = 12))+scale_x_discrete(breaks = seq(1985,2017,4))+
  labs(title = "Words appear over the years by applicatoin abstracts",x = "Year", y = "Frequency")
```


Plot from this analysis reveals other trends. Cancer is always higher than the rest of two. Gene is in a down trend in recent years while the other two are in the up trend in general. It will be interesting to see the trend of funding using this method to predict the next breakthrough in the health sector. Some of the parameters for text mining will need further refinement. Clustering/Classification can also be applied later on to get a better key word category. 
